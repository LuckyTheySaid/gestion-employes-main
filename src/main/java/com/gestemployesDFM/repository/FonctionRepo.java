...
public interface FonctionRepo extends JpaRepository<Fonction, Long> {
   ...
   @Query("SELECT new com.gestemployesDFM.dto.FonctionCountDTO(f.libelle, COUNT(e.id)) FROM Fonction f LEFT JOIN Employer e ON f.code = e.fonction.code GROUP BY f.libelle ORDER BY COUNT(e.id) DESC")
